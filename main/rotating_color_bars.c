...
void app_main(void)
{
    ...
    while (1) {
        ...
        uint16_t *buf = heap_caps_malloc(width * height * sizeof(uint16_t), MALLOC_CAP_SPIRAM);
        assert(buf);

        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                int bar = (x / bar_width + offset) % bar_count;
                buf[y * width + x] = colors[bar];
            }
        }
        ...
        offset++; // shift colors for next frame
        if (offset >= bar_count) offset = 0;

        vTaskDelay(pdMS_TO_TICKS(5000)); // 1000ms per frame
    }
